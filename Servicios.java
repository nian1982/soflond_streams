...
public class Servicios {
        ...
        public static Map<Area, Optional<Empleado>> empleadoMesPorArea(List<Empleado> empleados,
                ...
                return empleados.stream()
                                .filter(empleado -> tieneTurnosValidos(empleado.id(), turnos)) // Filtra empleados con
                                                                                               // turnos válidos
                ...
                // Para cada área, selecciona al empleado con más horas trabajadas
        }
        ...
        public static Map<Area, Map<TipoTurno, Integer>> desgloseHorasPorAreaYTipoTurno(List<Empleado> empleados,
                ...
                return empleados.stream() // Iniciamos un flujo de empleados
                                .filter(empleado -> tieneTurnosValidos(empleado.id(), turnos)) // Filtramos solo los
                                                                                               // empleados con
                                                                                               // turnos válidos
                                ...
                                turnos.stream()
                                                .filter(turno -> turno.empleadoId().equals(empleado.id())
                                                                && turno.tipo() != TipoTurno.AUSENCIA)
                                                .map(turno -> new AbstractMap.SimpleEntry<>(empleado.area(), turno)) // Crear
                                                                                                                     // un
                                                                                                                     // par
                                                                                                                     // con
                                                                                                                     // el
                                ...
                                .collect(Collectors.groupingBy( // Agrupar por área
                                                ...
                                                Collectors.groupingBy( // Dentro de cada área, agrupar por tipo de turno
                                                                ...
                                                                Collectors.summingInt(entry -> entry.getValue().horas()) // Sumar
                                                                                                                         // las
                                                                                                                         // horas
                                                                                                                         // de
                                                                                                                         // cada
        ...
        public static List<String> auditoriaCoberturaMinima(List<Empleado> empleados, List<RegistroTurno> turnos) {
                return turnos.stream()
                                ...
                                .entrySet().stream() // Convertir el mapa de grupos a un stream para poder
                                                     // porcesarloprocesarlo
                                .filter(entry -> entry.getValue().size() < 2) // Filtrar días donde hay menos de 2
                                                                              // empleados
